...
tf.app.flags.DEFINE_string('prefix', "elm-ws", """prefix for results""")
...
class Ranker:
    ...
    def _network(self, input_t, linout=False):
        size = self.ELM_Size
        reuse = True if self.network_built else False

        with tf.variable_scope("ELM", reuse=reuse):
            Welm = tf.get_variable("WELM", initializer=tf.truncated_normal([len(self.features), size], mean=0, stddev=self.ELM_init_sd), trainable=False)
            Belm = tf.get_variable("BELM", initializer=tf.truncated_normal([size], mean=0, stddev=self.ELM_init_sd), trainable=False)
            encoding = tf.nn.tanh(tf.matmul(input_t, Welm) + Belm)

        with tf.variable_scope("READ", reuse=reuse):
            WR = tf.get_variable("WR", initializer=tf.truncated_normal([size, 1], stddev=self.READ_init_sd))
            BR = tf.get_variable("BR", initializer=tf.truncated_normal([1], stddev=self.READ_init_sd))
    ...
    def _loss(self, pre, post, regularization):
        return tf.reduce_mean((2 - (self._network(pre) - self._network(post))) ** 2) + self.weight_cost * regularization

    def _performance(self, pre, post):
        # accuracy
        return tf.reduce_mean(tf.cast(tf.equal(tf.sign((self._network(pre) - self._network(post))), 1), tf.float32))
    ...
    def get_ranking(self, csv_file):
        ...
        outpre, outpost = self._network(pre, True), self._network(post, True)

        saver = tf.train.Saver(self.variable_to_save)
        with tf.Session() as S:
            saver.restore(S, self.checkpoint_name)
            return S.run([outpre, outpost])
...
def main(argv=None):
    ...
    if FLAGS.action == "select":
        ...
        R = Ranker(features,
                   ...
                   ELM_init_sd=5,
                   READ_init_sd=1e-3,
                   checkpoint_name='ckp/elm-ws.ckp'
                   )
        if FLAGS.retrain:
